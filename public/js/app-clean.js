class InventoryApp {
    ...
    setupEventListeners() {
        ...
        try {
            ...
            // Check if response is HTML (server not running or error page)
            const contentType = response.headers.get('content-type');
            if (!contentType || !contentType.includes('application/json')) {
                throw new Error('Server is not responding properly. Please ensure the server is running on http://localhost:3000');
            }
            ...
            if (error.name === 'TypeError' && error.message.includes('fetch')) {
                ...
                throw new Error('Server returned an error page instead of data. Please check if the server is running properly.');
            }
